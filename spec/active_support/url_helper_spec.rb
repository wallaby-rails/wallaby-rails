...
describe 'URL helpers', type: :helper do
  describe 'action_view/routing_url_for' do
    it 'returns url' do
      ...
      expect(super_method.call(parameters(controller: 'application', action: 'index').permit!)).to eq '/test/purpose'
    end
  end
  ...
  describe 'wallaby_engine.resources_path helper' do
    it 'returns url' do
      ...
      expect(helper.wallaby_engine.resources_path(parameters(action: 'index', resources: 'products').permit!)).to match '/admin/%23%3CActionController::Parameters'
    end
  end
end
