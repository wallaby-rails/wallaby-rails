...
describe 'Resources pages using mysql table' do
  ...
  describe '#index' do
    ...
    context 'sorting' do
      it 'renders collections' do
        get '/admin/all_mysql_types?sort=string%20desc'
        expect(response).to be_successful
        expect(response).to render_template :index
        expect(response.body).to include string
      end
    ...
    context 'keyword' do
      it 'renders collections' do
        get '/admin/all_mysql_types?q=van'
        expect(response).to be_successful
        expect(response).to render_template :index
        expect(response.body).to include string

        get '/admin/all_mysql_types?q=string:~van'
        expect(response).to be_successful
        expect(response).to render_template :index
        expect(response.body).to include string
      end
    ...
    context 'pagination' do
      it 'renders collections' do
        get '/admin/all_mysql_types?per=100'
        expect(response).to be_successful
        expect(response).to render_template :index
        expect(response.body).to include string

        get '/admin/all_mysql_types?page=1'
        expect(response).to be_successful
        expect(response).to render_template :index
        expect(response.body).to include string
      end
      ...
      context 'when not found' do
        it 'renders collections' do
          get '/admin/all_mysql_types?page=100'
          expect(response).to be_successful
          expect(response).to render_template :index
          expect(response.body).not_to include string
        end
